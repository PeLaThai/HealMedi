...
let buildCancelUrlEmail = (patientId, token, date, timeType, doctorId) => {
  let result = `${process.env.URL_REACT}/verify-cancel-book-appointment?token=${token}&patientId=${patientId}&date=${date}&timeType=${timeType}&doctorId=${doctorId}`;
  return result;
};
...
let postCancelBookAppointment = (data) => {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      if (!data.token || !data.doctorId || !data.patientId || !data.timeString || !data.doctorName || !data.date || !data.timeType) {
        resolve({
          errCode: 1,
          errMessage: 'Missing required parameter'
        })
        ...
        if (booking) {
          ...
          await emailService.sendCancelEmailForDoctor({
            ...
            time: data.timeString,
            doctorName: data.doctorName,
            redirectLink: buildCancelUrlEmail(data.patientId, data.token, data.date, data.timeType, data.doctorId),
          });
          resolve({
            errCode: 0,
  ...
  return new Promise(async (resolve, reject) => {
      try {
          ...
          if (!data.token || !data.patientId || !data.date || !data.timeType || !data.doctorId) {
              ...
              if (appointment) {
                  console.log(appointment, 'lich hen')
